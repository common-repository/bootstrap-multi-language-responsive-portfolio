	...
	class Portfolio_Post_Type {
		...
		**/   
        ...
        function add_portfolioposttype_metaboxes()
        {
            ...
            $client = isset($custom["client"][0]);
            $url = isset($custom["url"][0]);
            $url_text = isset($custom["url_text"][0]);
        ...
        function save_portfolioposttype_values( $post_id ){
          ...
          $pf_wpnonces = (isset($_REQUEST['pf_wpnonce']) ? $_REQUEST['pf_wpnonce'] : '');
        		
          if(! wp_verify_nonce( $pf_wpnonces, 'porftolio-nonce' ))
			return;
